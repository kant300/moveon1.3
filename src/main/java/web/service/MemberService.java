...
public class MemberService {
    ...
    // 1. 회원가입
    public boolean signup(MemberDto memberDto, String guestToken) {

        if (guestToken != null && !guestToken.isEmpty()) {

            try {
                // Bearer 제거
                String token = guestToken.replace("Bearer ", "");
                // 토큰에서 guest key 호출
                String tokenKey = jwtService.getGuestKey(token);
                // 게스트 정보 조회
                GuestDto guest = guestMapper.guestAddress(tokenKey);
                    // 게스트 일때 주소 / 관심 내역
                if (guest != null) {
                    memberDto.setMaddress1(guest.getGaddress1());
                    memberDto.setMaddress2(guest.getGaddress2());
                    memberDto.setMaddress3(guest.getGaddress3());
                    memberDto.setWishlist(guest.getWishlist());
                }
            } catch (Exception e) {
        ...
        if (result > 0 && guestToken != null) {
            // 사용한 토큰 / 키 가져와 삭제
            String token = guestToken.replace("Bearer ", "");
            String tokenKey = jwtService.getGuestKey(token);

            guestMapper.guestDelete(tokenKey);
        }
            return result > 0;
