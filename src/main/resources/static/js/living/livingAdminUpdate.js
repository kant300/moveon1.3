...
citySelect.addEventListener('change', () => {
  loadGuOptions(citySelect.value);
});
...
const trashFind = async (tcity, tgu) => {
  ...
  try {
    ...
    // 1. 시 옵션 가져오기
    loadCityOptions();
    citySelect.value = data.tcity;

    // 2. 구 옵션 가져오기
    loadGuOptions(data.tcity);
